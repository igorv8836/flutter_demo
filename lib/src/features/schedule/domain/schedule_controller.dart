...
class ScheduleController extends _$ScheduleController {
  late final ScheduleLocalDataSource _local;

  @override
  ScheduleState build() {
    _local = ScheduleLocalDataSource();
    final settings = ref.watch(settingsControllerProvider);
    if (_local.isEmpty) {
      final wake = _addDuration(settings.targetBedtime, settings.targetDuration);
      _local.seedDefaults(settings.targetBedtime, wake);
    }
    return ScheduleState(days: _local.read());
  }
